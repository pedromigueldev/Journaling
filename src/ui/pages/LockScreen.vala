...
namespace Journaling {
    public class LockScreen : Adw.NavigationPage {
        ...
        public LockScreen  (Adw.ApplicationWindow Window) {
            this.Window = Window;
        ...
        private async void _dialog_verify_password () {
            ...
            Gtk.Entry entry = new Gtk.Entry() { placeholder_text = "Type your password" };
            entry.set_visibility(false);

            var dialog_box = new Gtk.Box(Gtk.Orientation.VERTICAL, 15);
            dialog_box.append(_lock_icon());
            dialog_box.append(new Gtk.Label("Type your password for 'Journaling'") { css_classes = {"title-2"}, wrap = true, justify = Gtk.Justification.CENTER });
        ...
        private async void _dialog_create_password () {
            ...
            var dialog_box = new Gtk.Box(Gtk.Orientation.VERTICAL, 15);
            dialog_box.append(_lock_icon());
            dialog_box.append(new Gtk.Label("Tyepe your password for 'Journaling'") { css_classes = {"title-2"}, wrap = true, justify = Gtk.Justification.CENTER });
            dialog_box.append(entry);
            dialog.set_extra_child(dialog_box);
        ...
        private void _build_ui() {
            ...
            this._unlock_button.clicked.connect (() =>
                _dialog_verify_password.begin()
            );
            change_pass.clicked.connect(() =>
                _dialog_create_password.begin()
            );
